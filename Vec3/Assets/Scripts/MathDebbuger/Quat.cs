...
public struct Quat
{
	...
	public static Quat FromToRotation(Vec3 fromDirection, Vec3 toDirection)
	{
		...
		{
			Vec3 axis = Vec3.Cross(Vec3.Right, from); //Pruebo un eje arbitrario (porque podrían ser infinitos los
			//ejes a elegir entre dos vectores opuestos
		...
		return AngleAxis(angle, rotationAxis); //Rotar n grados en el eje obtenidoo
	}
	...
	public static Quat LerpUnclamped(Quat a, Quat b, float t)
	{
		...
		if (Dot(a, b) < 0f) //camino largo o camino corto
			//Interpolo hacia b o hacia -b?
		{
			result = new Quat(
				a.x + t * (-b.x - a.x),
				a.y + t * (-b.y - a.y),
	...
	public static Quat LookRotation(Vec3 forward, [DefaultValue("Vec3.Up")] Vec3 upwards)
	{
		...
		Vec3 right = Vec3.Cross(upwards, forward).normalized; //Obtengo dirección de la derecha con cross de adelante
		//y arriba
   ...
   M =  │ 2(xy+wz)      1-2(x²+z²)    2(yz-wx)    │
	...
	{
		q = Normalize(q);	//quaternion unitario

		//seno del angulo de rotación en X
		...
		{
			//En gimball lock, rotar en y o en z es lo mismo, asi que arbitrariamente asignamos la rotación a Y y z lo
			//dejamos en 0
			euler.x = Mathf.Sign(sinPitch) * 90f;
			euler.y = Mathf.Atan2(2f * (q.w * q.y - q.x * q.z), 1f - 2f * (q.y * q.y + q.x * q.x)) * Mathf.Rad2Deg;
			euler.z = 0f;
		}
		else
		{
	...
	public void ToAngleAxis(out float angle, out Vec3 axis)
	{
		...
		float sinHalf = Mathf.Sqrt(1f - w * w); //w = cos(θ/2)
		//sin(θ/2) = √(1 - cos²(θ/2)) = √(1 - w²)
